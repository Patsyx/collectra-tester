...
DEMO_ONLY=os.getenv('COLLECTRA_DEMO_ONLY','').strip().lower() in ('1','true','yes')
AI=AIService(ROOT)
        ...
        try:
            ...
            if path=='/api/products':
                ...
                if missing:
                    ...
                    return self.send_json({'error':'AI authentication requires all 4 photos before listing. Missing: '+', '.join(labels[x] for x in missing)},400)
                saved={}
                for key in required:
                ...
                ai_result=AI.verify(saved['front'][0],saved['back'][0],saved['edges'][0],saved['surface'][0],category,fields.get('title',''),[])
                if ai_result.get('verdict')=='Suspicious':
                    return self.send_json({
                        'error':'This listing was not authenticated and cannot be published. The AI flagged it as Suspicious — retake clear, well-lit photos of the same physical card and try again, or contact support for manual review.',
                        'ai':ai_result,
                    },422)
                image_url=saved['front'][1]
                con.execute('''INSERT INTO products(store_id,title,category,price,stock,condition,image_url,verified,description,ai_status,ai_confidence,ai_summary,ai_json,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
                    (st['id'],fields.get('title','Untitled'),category,float(fields.get('price','0')),int(fields.get('stock','1')),fields.get('condition','Like New'),image_url,int(st['verified']),fields.get('description','')[:1200],
                     ai_result.get('verdict',''),ai_result.get('confidence',0),ai_result.get('summary',''),json.dumps(ai_result),now_iso()))
                con.commit(); return self.send_json({'ok':True,'product_id':con.execute('SELECT last_insert_rowid() id').fetchone()['id'],'ai':ai_result})
            if path=='/api/verify':
                fields,files=parse_multipart(self)
                required=('front','back','edges','surface')
                missing=[key for key in required if key not in files]
